...
class NeRFRenderer(torch.nn.Module):
    ...
    def composite(self, model, rays, z_samp, coarse=True, sb=0):
        ...
        with profiler.record_function("renderer_composite"):
            ...
            if hasattr(model, 'get_color'):
                output_final = model.get_color(rgb_final,coarse)
                rgb_final =  output_final[:,-3:]
                feature_final = output_final[:,:-3]
    ...
    def forward(
        ...
        with profiler.record_function("renderer_forward"):
            ...
            z_coarse = self.sample_coarse(rays).to(rays.dtype)   # (B, Kc)
    ...
    def _format_outputs(
        ...
        weights, rgb, depth, feature = rendered_outputs
        if superbatch_size > 0:
            ...
            feature = feature.reshape(superbatch_size, -1, feature.shape[-1])
        ret_dict = DotMap(rgb=rgb, depth=depth,feature = feature)
        if want_weights:
