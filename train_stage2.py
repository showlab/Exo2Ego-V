...
from src.pipelines.pipeline_video import Exo2EgoVideoPipeline
...
def log_validation(
    ...
    tmp_denoising_unet = denoising_unet

    vae = vae.to(dtype=torch.float32)
    image_enc = image_enc.to(dtype=torch.float32)
    reference_unet = reference_unet.to(dtype=torch.float32)

    pipe = Exo2EgoVideoPipeline(
        vae=vae,
    ...
    gt = pixel_values[None].permute(0,2,1,3,4).detach().cpu()
    gt = (gt + 1.0) / 2.0
    video = torch.cat([gt, video], dim=0)

    results.append({"name": f"{eval_name}", "vid": video})

    vae = vae.to(dtype=torch.float16)
    image_enc = image_enc.to(dtype=torch.float16)
    reference_unet = reference_unet.to(dtype=torch.float16)
...
def main(cfg,args,conf):
    ...
    if args.use_pixelnerf:
        ...
        if args.nerf_path is not None:
            pixelNeRF.load_state_dict(torch.load(args.nerf_path).state_dict())
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(net):
                ...
                with torch.no_grad():
                    ...
                    ref_imgs = torch.cat(batch['condition_values'])[:, :, :, start_height:args.exo_height-start_height, start_width:args.exo_width-start_width].to(weight_dtype)
                    video_length = ref_imgs.shape[1]
                    t = rearrange(ref_imgs, "b f c h w -> (b f) c h w")
                    ...
                    if uncond_fwd:
                        clip_img_exo = torch.zeros_like(ref_imgs)
                    else:
                        clip_img_exo = (ref_imgs + 1) / 2.0
                    clip_img_exo = rearrange(clip_img_exo, "b f c h w -> (b f) c h w")
                ...
                if args.use_pixelnerf:
                    noisy_latents = torch.cat((noisy_latents, features_final[None].permute(0,2,1,3,4)),dim=1)
                ...
                rel_poses = rearrange(batch["rel_poses"][0], "b f h w -> (b f) (h w)")
                # ---- Forward! -----
                model_pred = net(
                    noisy_latents,
                    timesteps,
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.test_steps== 0:
                    if accelerator.is_main_process:
                        ...
                        rel_poses = rearrange(all_validation_values['rel_poses'], "b f h w -> (b f) (h w)").to(accelerator.device)
                        sample_dicts = log_validation(
                            ...
                            poses = rel_poses,
                            condition_values = validation_values_exo[:, 0, :, ...][:, :,:, start_height:args.exo_height-start_height, start_width:args.exo_width-start_width],
                        ...
                        for sample_id, sample_dict in enumerate(sample_dicts):
                            ...
                            vid = sample_dict["vid"]
                            temp_dir = os.path.join(args.output_dir,"test_vids")
                ...
                if global_step % args.validation_steps == 0:
                    ...
                    if accelerator.is_main_process:
                        ...
                        rel_poses = rearrange(batch["rel_poses"][0], "b f h w -> (b f) (h w)")
                        condition_values_ = torch.cat(batch['condition_values'])[:, :,:, start_height:args.exo_height-start_height, start_width:args.exo_width-start_width]
                        sample_dicts = log_validation(
                            vae=vae,
                            image_enc=image_enc,
                            net=net,
                        ...
                        for sample_id, sample_dict in enumerate(sample_dicts):
                            sample_name = sample_dict["name"]
                            vid = sample_dict["vid"]
                            temp_dir = os.path.join(args.output_dir,"validation_vids")
                            if not os.path.exists(temp_dir):
                                os.makedirs(temp_dir)
                            out_file = Path(
                                f"{temp_dir}/{global_step:06d}-{sample_name}.gif"
